...
export default function QuoteDocumentsPage() {
  ...
  const [staticQuotes, setStaticQuotes] = useState<StaticQuote[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const [isAuthenticated, setIsAuthenticated] = useState(false);
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [documentToDelete, setDocumentToDelete] = useState<QuoteDocument | null>(null);
  ...
  const fetchDocuments = async () => {
    try {
      const response = await fetch('/api/admin/quote-documents');
      ...
      console.error('DB 견적서 로딩 오류:', err);
    }
  };

  const fetchStaticQuotes = async () => {
    try {
      const response = await fetch('/api/admin/static-quotes');
      const result = await response.json();
      ...
      setStaticQuotes(result.data || []);
    } catch (err) {
      console.error('정적 견적서 로딩 오류:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig: Record<string, { text: string; color: string }> = {
      ...
      static: { text: 'HTML', color: 'bg-purple-500' }
    };
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/quote-documents/${documentToDelete.id}`, {
        method: 'DELETE'
      });
  ...
  const totalCount = documents.length + staticQuotes.length;
            ...
            <div className="flex items-start justify-between gap-4 mb-3">
              <div>
                <h1 className="text-4xl lg:text-5xl font-black text-white mb-3">
                  견적서 <span className="text-green">관리</span>
                </h1>
                <p className="text-lg text-gray-400">견적서를 생성, 조회, 수정, 삭제하세요.</p>
              </div>
          ...
          >
            <div className="flex-1 bg-black-light border-2 border-gray-800 rounded-xl p-6 hover:border-green/30 transition-colors">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-gray-400 text-sm font-medium mb-1">총 견적서</p>
                  <p className="text-5xl font-black text-white">{totalCount}</p>
                  <p className="text-sm text-gray-500 mt-2">
                    DB: {documents.length} / HTML: {staticQuotes.length}
                  </p>
                </div>
          ...
          >
            <div className="px-6 py-5">
              <h3 className="text-2xl font-bold text-white mb-6">
                {activeTab === 'static' ? 'HTML 견적서 목록' : 'DB 견적서 목록'}
              </h3>
              ...
              {activeTab === 'static' && (
                <>
                  {staticQuotes.length === 0 ? (
                    <p className="text-gray-400 text-center py-12">HTML 견적서가 없습니다.</p>
                  ) : (
                    <div className="space-y-4">
                      {staticQuotes.map((quote, index) => (
                        <motion.div
                          ...
                          className="bg-black border-2 border-gray-800 rounded-xl p-6 hover:border-purple-500/50 transition-all"
                        >
                          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                            <div className="flex-1">
                              <div className="flex items-center gap-3 mb-2">
                                <h4 className="text-lg font-bold text-white">{quote.title}</h4>
                                {getStatusBadge('static')}
                              </div>
                              <p className="text-sm text-gray-500">{quote.filename}</p>
                            </div>
                            <div className="flex flex-col items-end gap-2">
                              {quote.date && (
                                <span className="text-xs text-gray-500">{formatDate(quote.date)}</span>
                              )}
              ...
              {activeTab === 'db' && (
                <>
                  {documents.length === 0 ? (
                    <div className="text-center py-12">
                      <p className="text-gray-400 mb-4">DB에 저장된 견적서가 없습니다.</p>
                      <Link
                        href="/admin/quotes/new"
                ...
                >
                  <div className="bg-black-light border-2 border-red-500/30 rounded-2xl max-w-md w-full p-6">
                    <h3 className="text-2xl font-black text-white mb-4">
                      견적서 <span className="text-red-400">삭제</span>
                    </h3>
                    <p className="text-gray-400 mb-6">
                      &quot;{documentToDelete.title}&quot; 견적서를 삭제하시겠습니까?
                      <br />
